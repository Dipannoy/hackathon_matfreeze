...
def save_representations(modalities_encoders, encoders, data_loader, epoch, save_dir, device, crystal_arch='potnet'):
    ...
    Args:
        modalities_encoders: List of modality names
        encoders: Dictionary of encoder models
        data_loader: DataLoader to iterate through
        epoch: Current epoch number
        save_dir: Directory to save representations
        device: Device to run on
        crystal_arch: Architecture type
    """
    # Set models to eval mode
    not_needed = {modality: nn.Identity() for modality in modalities_encoders}
    switch_mode(modalities_encoders, encoders, not_needed, mode='eval')
            ...
            if crystal_arch == 'cgcnn':
                for modality in modalities_encoders:
                    if modality == 'crystal':
                        ...
                        embeddings = encoders[modality](*crystal_var)
                    else:
                        data_input = data[modality].to(device)
                        embeddings = encoders[modality](data_input)
                    
                    representations_dict[modality].append(embeddings.cpu())
            else:
                # For other architectures (potnet, etc.)
                modalities_all = modalities_encoders
                data = tensros_to_device(modalities_all, data, device)
                
                for modality in modalities_encoders:
                    embeddings = encoders[modality](data[modality])
                    representations_dict[modality].append(embeddings.cpu())
